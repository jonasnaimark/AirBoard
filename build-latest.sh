...
sed -i '' 's/ExtensionBundleVersion="[^"]*"/ExtensionBundleVersion="4.17.15"/g' temp-package/CSXS/manifest.xml

echo "✅ Production files cleaned"

# Navigate to temp directory and create ZXP
cd temp-package
/Applications/ZXPSignCmd -sign . ../dist/AirBoard-v4.17.15.zxp ../new-cert.p12 mypassword
...
if [ -f "dist/AirBoard-v4.17.15.zxp" ]; then
    echo "✅ SUCCESS: ZXP created at dist/AirBoard-v4.17.15.zxp"
    ls -la dist/AirBoard-v4.17.15.zxp
else
