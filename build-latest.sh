...
sed -i '' 's/ExtensionBundleVersion="[^"]*"/ExtensionBundleVersion="4.16.33"/g' temp-package/CSXS/manifest.xml

echo "✅ Production files cleaned"

# Navigate to temp directory and create ZXP
cd temp-package
../ZXPSignCmd -sign . ../dist/AirBoard-v4.16.33.zxp ../new-cert.p12 mypassword
...
if [ -f "dist/AirBoard-v4.16.33.zxp" ]; then
    echo "✅ SUCCESS: ZXP created at dist/AirBoard-v4.16.33.zxp"
    ls -la dist/AirBoard-v4.16.33.zxp
else
